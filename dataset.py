...
def dataset_to_dict(data_path: str = data_path):
    ...
    subsets = {i: [] for i in range(num_classes)}
...
def import_dataset(
    ...
    # Auto-detect subclasses from data if not specified
    if subclasses is None:
        class_names = get_class_names(data_path)
        subclasses = list(range(len(class_names)))
...
class DatasetLung(Dataset):
    def __init__(self,
        ...
        self.subclasses = subclasses if subclasses is not None else []
        self.cutoffs = self._cutoffs(self.subclasses, cond_drop_prob)
    ...
    def _subclasses(self, data_dict: dict, subclasses: list):
        # Collect all images for unconditional class (classifier-free guidance)
        all_images = []
        for k in data_dict.keys():
            all_images += data_dict[k]
        ...
        data_dict[0] = all_images
        return data_dict

    def _cutoffs(self, subclasses, cond_drop_prob=0.5):
        # Only include classes that have data
        n_active = sum(1 for k in self.data_dict if len(self.data_dict[k]) > 0 and k != 0)
        if n_active == 0:
            n_active = 1
        probs=[cond_drop_prob/n_active for n in range(n_active)]
