...
class Trainer(object):
    ...
    def _get_vae(self):
        """Unwrap VAE whether or not it's wrapped by Accelerator/DDP."""
        return self.vae.module if hasattr(self.vae, 'module') else self.vae
    ...
    def load(self, milestone):
        ...
        try:
            ...
            print("Warning: scheduler state mismatch (checkpoint from different branch), skipping scheduler load.")
    ...
    def train_loop(self, imgs, masks):
        with torch.no_grad():
            imgs=self._get_vae().encode(imgs).latent_dist.sample()/50
    ...
    def eval_loop(self):
        ...
        if self.accelerator.is_main_process:
            ...
            ema_unwrapped = self.ema.module if hasattr(self.ema, 'module') else self.ema
            ema_unwrapped.update()

            if self.step != 0 and self.step % self.save_and_sample_every == 0:
                ema_unwrapped.ema_model.eval()
                ...
                with torch.no_grad():
                    ...
                    ema_model = self.ema.module.ema_model if hasattr(self.ema, 'module') else self.ema.ema_model
                    z = ema_model.sample(z,test_masks[:self.num_samples])*50
