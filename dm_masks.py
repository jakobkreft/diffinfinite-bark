...
class Trainer(object):
    ...
    def train_loop(self, imgs, labels):
        with torch.no_grad():
            imgs = self._get_vae().encode(imgs).latent_dist.sample()
        ...
        if self.accelerator.is_main_process:
            ...
            if self.step != 0 and self.step % self.save_and_sample_every == 0:
                ...
                with torch.no_grad():
                    ...
                    # Sample one mask per class
                    image_classes = torch.arange(self.num_labels).cuda()[:self.num_samples]

                    z = ema_unwrapped.ema_model.sample(image_classes, cond_scale=self.cond_scale)
                    ln = self.latent_normalize.module if hasattr(self.latent_normalize, 'module') else self.latent_normalize
                    z = ln.un_normalize(z)

                    y = self._get_vae().decode(z).sample
